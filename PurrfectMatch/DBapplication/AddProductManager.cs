...
namespace DBapplication
{
    ...
    public partial class AddProductManager : Form
    {
        ...
        public AddProductManager(string user)
        {
            InitializeComponent();
        ...
        {
            DateTime proddate, exdate;
            if (product_name_textbox.Text == "" || type_textbox.Text == "" || price_textbox.Text == "" || center_textbox.Text == "" || stock_textbox.Text == "" || description_textbox.Text == "" || proddate_textbox.Text == "" || exdate_textbox.Text == "")
            {
            ...
            if (!DateTime.TryParse(proddate_textbox.Text, out proddate) || !DateTime.TryParse(exdate_textbox.Text, out exdate))
            {
                MessageBox.Show("Incorrect way to insert dates, please input them in the form yyyy-mm-dd");
                return;
            }
            ...
            bool successConversion_1 = int.TryParse(stock, out _);
            if (successConversion_1 == false)
            {
                MessageBox.Show("Please, insert integer values for stock");
            }
            else
            {
                int r = controllerObj.InsertProduct(product_name_textbox.Text, description_textbox.Text, Convert.ToInt32(stock_textbox.Text), proddate, exdate, price, center_textbox.Text, type_textbox.Text);
                if (r == 0)
                {
